...
export default function NewMeetingPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    const { data, error } = await supabase.from('events').insert({
      ...
      checkin_open: form.checkin_open, created_by: user.id,
    }).select().single()
    ...
    <>
      ...
      <form onSubmit={handleCreate}>
        <table style={{ border: 'none', width: '100%', maxWidth: 500 }}>
          <tbody>
            <Row label="title *"><input type="text" value={form.title} onChange={e => update('title', e.target.value)} style={{ width: '100%' }} placeholder="Builders Club — Week 12" /></Row>
            <Row label="date *"><input type="datetime-local" value={form.event_date} onChange={e => update('event_date', e.target.value)} style={{ width: '100%' }} /></Row>
            <Row label="description"><textarea value={form.description} onChange={e => update('description', e.target.value)} rows={3} style={{ width: '100%' }} /></Row>
            <Row label="check-in"><label><input type="checkbox" checked={form.checkin_open} onChange={e => update('checkin_open', e.target.checked)} /> open immediately</label></Row>
          </tbody>
        </table>
        <hr />
